...
Nhóm mục tiêu thứ hai là phần trọng tâm của luận văn, bao gồm huấn luyện và so sánh đối chứng trên hai tầng kiến trúc. Tầng cơ sở gồm một teacher (EfficientNet-B4) và ba student cổ điển đại diện cho các paradigm lightweight truyền thống; tầng SOTA mở rộng sang ba teacher tiên tiến (EfficientNetV2-M, ConvNeXtV2-Base, MaxViT-Base) và ba student tối ưu cho thiết bị di động (MobileNetV4-Conv-Medium, FastViT-SA12, EfficientFormerV2-S2). Mỗi kiến trúc student được huấn luyện song song theo hai điều kiện hoàn toàn kiểm soát — có KD và không có KD — nhằm tách biệt tác động của phương pháp chưng cất khỏi các yếu tố nhiễu. Bên cạnh đó, đề tài phân tích tương quan giữa chất lượng teacher (AUPRC teacher) và mức độ cải thiện của student (ΔAUPRC), một câu hỏi nghiên cứu bổ sung có giá trị hướng dẫn thiết kế thực nghiệm KD trong tương lai.
...
**Kiến trúc teacher–student hai tầng.** Tầng cơ sở sử dụng EfficientNet-B4 (~19M tham số) làm teacher và ba student cổ điển đại diện cho các paradigm CNN truyền thống. Tầng SOTA nâng cấp teacher lên ba kiến trúc tiên tiến — EfficientNetV2-M, ConvNeXtV2-Base, MaxViT-Base — và cặp với ba student tối ưu cho thiết bị di động. Sự phân tầng này cho phép đồng thời trả lời hai câu hỏi: *KD có hiệu quả hơn trên student SOTA không?* và *Teacher mạnh hơn có tạo ra student tốt hơn không?*

| Tier | Teacher | Student | Paradigm |
|---|---|---|---|
| Baseline | EfficientNet-B4 (~19M) | EfficientNet-B0 (~5,3M) | Compound Scaling |
| Baseline | EfficientNet-B4 | MobileNetV3-Large (~5,4M) | NAS-optimized CNN |
| Baseline | EfficientNet-B4 | MobileViT-S (~5,6M) | Hybrid CNN-Transformer |
| SOTA | EfficientNetV2-M, ConvNeXtV2-Base, MaxViT-Base | MobileNetV4-Conv-Medium (~9M) | Depthwise-separable CNN |
| SOTA | EfficientNetV2-M, ConvNeXtV2-Base, MaxViT-Base | FastViT-SA12 (~11M) | CNN + Reparam |
| SOTA | EfficientNetV2-M, ConvNeXtV2-Base, MaxViT-Base | EfficientFormerV2-S2 (~12M) | Attention-CNN hybrid |

Tất cả mô hình chia sẻ cùng một classification head (Global Average Pooling → Dropout → Dense(1)) và áp `sigmoid` tại inference. Backbone được load từ thư viện `timm` (SOTA tier yêu cầu `timm≥1.0`). Teacher luôn đóng băng trong quá trình KD.
...
trong đó `L_hard = FocalLoss(z_student, y_true)` (γ=2, α=0,25) [28] cung cấp tín hiệu từ nhãn cứng; `L_soft = BCE(σ(z_student/T), σ(z_teacher/T))` truyền "dark knowledge" từ phân phối mềm của teacher với nhiệt độ `T = 4,0`; hệ số cân bằng `α = 0,3` phân bổ 30% nhãn cứng và 70% nhãn mềm. Khi chạy nhánh baseline, `α = 1,0` và không load teacher, đảm bảo tính kiểm soát hoàn toàn của thí nghiệm.

**Thiết kế thực nghiệm so sánh đối chứng.** Điểm mấu chốt của thiết kế là nguyên tắc *ceteris paribus*: mỗi kiến trúc student được huấn luyện song song theo hai nhánh (có KD / không KD) với cùng dữ liệu, seed ngẫu nhiên, siêu tham số và quy trình đánh giá — chỉ khác ở hàm mất mát. Tổng số lượt huấn luyện theo toàn bộ ma trận gồm 4 teacher × 5 folds (= 20 lượt), 6 student baseline × 5 folds (= 30 lượt), và xấp xỉ 12 cặp teacher–student KD × 5 folds (≈ 60 lượt). Hiệu quả KD được định lượng qua **ΔpAUC** và **ΔAUPRC** (KD trừ baseline), được kiểm định bằng Paired t-test (p < 0,05) trên 5 fold.

**Triển khai biên.** Mô hình tốt nhất sau khi chọn lọc được export sang định dạng **ExecuTorch .pte** (FP32) phục vụ Android on-device inference. Hiệu năng thực tế được đo trực tiếp trên thiết bị **Pixel 6a** (ARM Cortex-A55/A76, 4 threads), gồm độ trễ trung vị (ms), kích thước file (MB) và throughput (FPS). Phân tích Pareto giữa AUPRC và latency được thực hiện để xác định frontier tối ưu, đồng thời trả lời câu hỏi thực tiễn: *mô hình nào đạt cân bằng tốt nhất giữa độ chính xác lâm sàng và khả năng chạy thời gian thực trên điện thoại phổ thông?*
...
**Huấn luyện.** Tất cả mô hình sử dụng cùng thiết lập tối ưu hóa: AdamW với learning rate phân tầng (1e-4 cho backbone, 1e-3 cho classification head), cosine annealing scheduler, và 50 epoch tối đa. Checkpoint được lưu theo val pAUC@TPR≥80%. Teacher được huấn luyện trước với Focal Loss thuần [28] (không có soft label). Student được huấn luyện sau, sử dụng hàm mất mát KD (nhánh KD) hoặc Focal Loss thuần (nhánh baseline). Các SOTA teacher (EfficientNetV2-M, ConvNeXtV2-Base, MaxViT-Base) yêu cầu `timm≥1.0` và phân bổ VRAM cao hơn. Toàn bộ thực nghiệm chạy trên cụm UIT Slurm (GPU L40, MPS mode), cấu hình qua Hydra, submit qua `bash slurm/submit.sh`.

**Đánh giá.** Sau mỗi lần huấn luyện, best checkpoint được tải lại và chạy inference trên tập test độc lập để tính: pAUC@TPR≥80% (metric chính thức ISIC 2024), AUPRC (headline metric ở prevalence ~0,4%), AUC-ROC, sensitivity, specificity và F1-score. Ngưỡng quyết định được chọn theo Youden's J statistic. Kết quả 5 fold được tổng hợp theo mean ± std; Paired t-test kiểm định ý nghĩa thống kê của ΔpAUC và ΔAUPRC. Đánh giá được tổ chức theo ba tầng độc lập: *(i) in-domain* trên ISIC 2024 + PAD-UFES-20 test split; *(ii) cross-domain* trên HAM10000 [17] (ảnh dermoscopic, không tham gia huấn luyện); *(iii) fairness* trên Fitzpatrick17k [18], phân nhóm theo sắc tố da Fitzpatrick I–VI (sáng I–II / trung bình III–IV / tối V–VI), đo khoảng cách hiệu năng lớn nhất–nhỏ nhất giữa các nhóm.
...
Về khoa học, kết quả trung tâm là **ma trận so sánh KD vs baseline** đầy đủ cho tất cả các cặp teacher–student (4 teacher × 6 student / 2 tier, 5-fold CV), trình bày ΔpAUC và ΔAUPRC (mean ± std) kèm kết quả Paired t-test. Trên cơ sở đó, đề tài đề xuất phân tích tương quan giữa chất lượng teacher (AUPRC teacher) và mức độ cải thiện của student (ΔAUPRC), cung cấp bằng chứng thực nghiệm trả lời câu hỏi *"teacher tốt hơn có dẫn đến KD hiệu quả hơn không?"*. Kết quả kiểm chứng 3 tầng — in-domain, cross-domain (HAM10000) và fairness (Fitzpatrick17k) — hoàn thiện đánh giá toàn diện về khả năng tổng quát hóa và tính công bằng của mô hình được đề xuất.

Về kỹ thuật, đề tài cung cấp **pipeline hoàn chỉnh và tái lập được**: mã nguồn Python + Hydra, script Slurm cho cụm GPU, và bộ artifact chuẩn hóa (`aggregated.json/md`, `predictions.csv`, `test_metrics.json`) cho phép tái tính toán mọi metric mà không cần chạy lại inference. Bổ sung vào đó là **báo cáo triển khai Android**: latency trung vị, throughput và kích thước .pte trên Pixel 6a cùng phân tích Pareto — cơ sở thực tiễn để người dùng lựa chọn mô hình phù hợp theo ràng buộc thiết bị. Toàn bộ sản phẩm này cùng với luận văn được hoàn thành thành văn là đầu ra khép kín của nghiên cứu.
...
|---|---|---|
| 1 | Xây dựng pipeline huấn luyện; tiền xử lý dữ liệu ISIC 2024 + PAD-UFES-20; thiết lập 5-fold CV patient-disjoint | 06/2026 |
| 2 | Huấn luyện 4 mô hình teacher (EfficientNet-B4, EfficientNetV2-M, ConvNeXtV2-Base, MaxViT-Base) — 5 fold CV mỗi mô hình | 06/2026 |
| 3 | Huấn luyện 6 student theo 2 nhánh (KD + baseline): Tier 1 cặp với B4; Tier 2 (MobileNetV4/FastViT/EfficientFormerV2) cặp với 3 SOTA teacher — hoàn thiện toàn bộ ma trận | 06–07/2026 |
| 4 | Export ExecuTorch .pte; benchmark on-device trên Pixel 6a (latency, FPS, kích thước); phân tích Pareto (AUPRC vs latency) | 07/2026 |
| 5 | Đánh giá chéo miền trên HAM10000; phân tích công bằng trên Fitzpatrick17k (nhóm sắc tố da I–VI) | 07–08/2026 |
| 6 | Tổng hợp kết quả 5-fold, kiểm định Paired t-test, phân tích tương quan chất lượng teacher ↔ ΔAUPRC; hoàn chỉnh bộ artifact (`aggregated.json/md`, `predictions.csv`) | 08/2026 |
| 7 | Viết luận văn (chương lý thuyết, chương thực nghiệm, chương kết quả và thảo luận) | 08–10/2026 |
| 8 | Hoàn thiện, nộp luận văn và chuẩn bị bảo vệ | 11/2026 |

> *Ghi chú:* Các giai đoạn 2–3 phụ thuộc vào hàng đợi GPU của cụm UIT Slurm (chia sẻ) và có thể chạy gối nhau khi tài nguyên cho phép. Giai đoạn 7 có thể bắt đầu song song với giai đoạn 5–6 cho các chương không phụ thuộc vào kết quả thực nghiệm.
