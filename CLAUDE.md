...
The wrapper does `mkdir -p logs` before `sbatch` (Slurm 23 silently drops output if `logs/` doesn't exist). Every `*.slurm` script sources `slurm/_lib.sh` which provides `set -euo pipefail`, a fallback `tee` log at `logs/<job>_<jobid>_runtime.log`, a diagnostic header, and helpers `load_python_env` / `acquire_gpu` / `setup_mps`. **GPU jobs request an exclusive whole GPU (`--gres=gpu:l40:1`), not an MPS slice** — see the "MPS oversubscription" gotcha; `setup_mps` is now only for scripts deliberately switched back to `--gres=mps:l40:N`. See `docs/SLURM.md` for the full guide.
...
- NOTE (2026-06-23): the maxvit POC failure may have been **MPS contention OOM, not a real cuDNN bug** — see the next gotcha. Re-test maxvit now that GPU jobs are exclusive; the cuDNN error may simply not recur.

### MPS oversubscription → OOM; GPU jobs now request an exclusive whole GPU (added 2026-06-23)

Real training repeatedly OOM'd at the **first CUDA allocation** (jobs 32550/32552/32558: convnextv2/efficientnetv2_m, all `torch.OutOfMemoryError` / "GPU 0 ... 3 MiB free") even though the models are small. Root cause is **not** code/model: Slurm's `gres/mps` plugin schedules by **compute-%, ignoring GPU memory**, so it co-located our `--gres=mps:l40:N` jobs onto a physical GPU (always GPU 0) that another user's ~29 GiB job had filled — MPS provides no memory isolation → OOM. `acquire_gpu` honored Slurm's pre-set `CUDA_VISIBLE_DEVICES=0` without re-checking free VRAM. Empty GPUs (2 & 6, ~45 GiB free) sat unused because Slurm had pinned us.

Fix (the queue-don't-contend rule, applied to all GPU scripts `02/03/11/12/13/14/20` + `_template`): **`#SBATCH --gres=gpu:l40:1`** (exclusive whole GPU) instead of `mps:l40:N`, and **drop `setup_mps`** (MPS unneeded when exclusive). Confirmed available: `scontrol show config | grep -i gres` → `GresTypes = gpu,mps`; `srun --gres=gpu:l40:1 … nvidia-smi` → `queued and waiting` (accepted). Behavior now: Slurm holds the job `PD` until a GPU is fully free, then it runs alone → **never shares/contends VRAM, never steals** from a running job. Concurrency is bounded by the 5-concurrent-jobs limit, not the 20-MPS budget. Don't reintroduce `--gres=mps` for training without a deliberate reason — MPS co-location is what caused the OOMs.
